...
import { handleAddToQueue, QueueEntry } from "./handlers/QueueManagement/addToQueue.js";
...
function broadcastPlayerStats(io: Server) {
...
app.prepare().then(() => {
  ...
  io.on("connection", (socket: any) => {
    ...
    // Queue management
    socket.on("joinQueue", (data: { modifier?: string }) => {
    ...
    // Rematch handling
    socket.on("requestRematch", (data: { sessionId: string }) =>
