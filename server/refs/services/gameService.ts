...
export class GameService {
  async startNewRound(player1: QueueItem, player2: QueueItem): Promise<void> {
    ...
    debugLog(
      `[ROUND] Starting new round ${roundId} for players ${player1.sessionId} and ${player2.sessionId}`
    );
    ...
    try {
      ...
      debugLog(
        `[ROUND] Selected image ${roundData.imageId} for round ${roundId}`
      );
        ...
        if (roundLock.playersReady.size === 2) {
          ...
          [player1, player2].forEach((player) => {
            if (player.socket?.connected) {
              debugLog(
                `[ROUND] Sending round data to player ${player.sessionId}`
              );
              player.socket.emit("startRound", roundLock.roundData);
            } else {
              debugLog(
                `[ROUND] Player ${player.sessionId} socket not connected`
              );
            }
          });
          ...
          setTimeout(() => {
            ...
            debugLog(
              `[ROUND] Cleaned up round ${roundId} and match ${matchId} after timeout`
            );
          }, 300000); // 5 minutes
        }
      };
      ...
      debugLog(
        `[ROUND] Player 1 (${player1.sessionId}) socket status: ${
          player1.socket ? "exists" : "null"
        }, connected: ${player1.socket?.connected || false}`
      );
      debugLog(
        `[ROUND] Player 2 (${player2.sessionId}) socket status: ${
          player2.socket ? "exists" : "null"
        }, connected: ${player2.socket?.connected || false}`
      );
      ...
      if (player1.socket?.connected) {
        ...
        debugLog(
          `[ROUND] Player 1 (${player1.sessionId}) socket not connected`
        );
      }
      ...
      if (player2.socket?.connected) {
        ...
        debugLog(
          `[ROUND] Player 2 (${player2.sessionId}) socket not connected`
        );
      }
      ...
      if (readyCount === 2) {
        debugLog(
          `[ROUND] Both players connected, round ${roundId} should start automatically`
        );
      }
    } catch (err) {
  ...
  async restartSession(sessionId: string, partnerId: string): Promise<void> {
    ...
    if (player1 && player2) {
      ...
      debugLog(
        `[RESTART] Could not find both players for restart: ${sessionId}, ${partnerId}`
      );
    }
  }
}
