...
export async function handleAddToQueue(queue, socket, gameRooms, io, modifier) {
    ...
    if (matchIndex !== -1) {
        ...
        console.log(`Matched users: ${userId} and ${partnerSessionId} in match ${sessionId} (modifier: ${normalizedModifier}, timedMode: ${timedMode})`);
        // Create a new game room for the matched users
        const newGame = await GameType.generateNewGame(sessionId, userId, partnerSessionId, timedMode);
...
function addToQueue(queue, socket, modifier) {
