...
export function calculateTimeBonus(playerSubmitTime, opponentSubmitTime) {
...
export async function handleSubmitGuess(socket, xCoor, yCoor, sessionId, socketId, gameRooms) {
    ...
    if (game.player1Guess != null && game.player2Guess != null) {
        ...
        const player1BasePoints = calculatePoints(game.player1Guess.x, game.player1Guess.y, game.answer.x, game.answer.y);
        const player2BasePoints = calculatePoints(game.player2Guess.x, game.player2Guess.y, game.answer.x, game.answer.y);
        ...
        (async () => {
            try {
                ...
                if (game.imageId) {
                    ...
                    await collection.updateOne({ _id: new ObjectId(game.imageId) }, {
                        $inc: {
        ...
        setTimeout(async () => {
            ...
            if (game.currentRoundIndex >= 4) {
                ...
                const winner = game.player1Points > game.player2Points
                    ? game.player1Id
                    : game.player2Id;
