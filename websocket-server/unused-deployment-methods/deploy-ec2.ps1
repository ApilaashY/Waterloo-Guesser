...
# Much simpler than ECS - just a single EC2 instance
...
function New-KeyPair {
    ...
    if ($LASTEXITCODE -eq 0) {
        ...
        Write-ColorOutput "✅ Key pair created: $KeyPairName.pem" $Green
...
function New-SecurityGroup {
    Write-ColorOutput "🛡️ Creating Security Group..." $Blue
...
/opt/aws/bin/cfn-signal -e 0 --stack none --resource none --region $Region; echo "Setup complete"
"@
...
"@
    ...
    & scp -i "$KeyPairName.pem" -o StrictHostKeyChecking=no -r server.js package.json handlers types lib Dockerfile $deployFiles "ec2-user@$($InstanceInfo.PublicIp):/home/ec2-user/waterloo-guesser-ws/"
    ...
    if ($LASTEXITCODE -eq 0) {
        ...
        & ssh -i "$KeyPairName.pem" -o StrictHostKeyChecking=no "ec2-user@$($InstanceInfo.PublicIp)" "chmod +x /home/ec2-user/waterloo-guesser-ws/deploy-app.sh; /home/ec2-user/waterloo-guesser-ws/deploy-app.sh"
        
        if ($LASTEXITCODE -eq 0) {
            Write-ColorOutput "✅ Application deployed successfully!" $Green
        } else {
            Write-ColorOutput "❌ Deployment failed" $Red
            Write-ColorOutput "💡 Try connecting via SSH to debug: ssh -i $KeyPairName.pem ec2-user@$($InstanceInfo.PublicIp)" $Yellow
        }
    } else {
...
function Test-Deployment {
    ...
    try {
        ...
        if ($response.status -eq "healthy") {
            ...
            Write-ColorOutput "⚠️ Health check returned unexpected response" $Yellow
        }
    } catch {
...
Write-ColorOutput "`n🎯 Starting Simple EC2 WebSocket Deployment" $Blue
Write-ColorOutput "This will create a single EC2 instance for your WebSocket server" $Blue
Write-ColorOutput "Cost: ~`$10-15/month (much less than ECS setup)`n" $Green
...
Write-ColorOutput "   Instance ID: $($instanceInfo.InstanceId)" $Reset
Write-ColorOutput "   Public IP: $($instanceInfo.PublicIp)" $Reset
Write-ColorOutput "   Instance Type: $InstanceType" $Reset
Write-ColorOutput "   Region: $Region" $Reset
Write-ColorOutput "`n🌐 WebSocket Endpoint:" $Blue
...
Write-ColorOutput "`n🔑 SSH Access:" $Blue
Write-ColorOutput "   ssh -i $KeyPairName.pem ec2-user@$($instanceInfo.PublicIp)" $Yellow
Write-ColorOutput "`n📝 Next Steps:" $Blue
Write-ColorOutput "   1. Update your Vercel environment variable:" $Reset
Write-ColorOutput "      NEXT_PUBLIC_WEBSOCKET_URL=http://$($instanceInfo.PublicIp):3001" $Green
Write-ColorOutput "   2. Test your multiplayer functionality" $Reset
Write-ColorOutput "   3. Monitor logs: ssh to server and run 'docker-compose logs -f'" $Reset
Write-ColorOutput "`n💰 Estimated Monthly Cost: ~`$10-15" $Green
Write-ColorOutput "================================================" $Blue
